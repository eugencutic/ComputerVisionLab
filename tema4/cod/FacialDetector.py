...
class FacialDetector:
    ...
    def get_positive_descriptors(self):
        ...
        for i in range(num_images):
            ...
            if self.params.use_flip_images:
                descr = hog(np.fliplr(img), pixels_per_cell=(self.params.dim_hog_cell, self.params.dim_hog_cell),
                            cells_per_block=(2, 2), feature_vector=True)
                positive_descriptors.append(descr)
    ...
    def get_negative_descriptors(self):
        ...
        for i in range(num_images):
            ...
            for i in range(len(x)):
                patch = img[y[i]: y[i] + self.params.dim_window, x[i]: x[i] + self.params.dim_window]
    ...
    def intersection_over_union(self, bbox_a, bbox_b):
        ...
        iou = inter_area / float(box_a_area + box_b_area - inter_area + 0.000001)
    ...
    def run(self, return_descriptors=False):
        ...
        for i in range(num_test_images):
            ...
            while True:
                if num_scaling > 0:
                    img = cv.resize(img, (0, 0), fx=self.params.scaling_ratio, fy=self.params.scaling_ratio)
                    scale = scale * self.params.scaling_ratio
                num_scaling += 1
                if min(img.shape[0], img.shape[1]) < self.params.dim_window:
                    break

                hog_descr = hog(img, pixels_per_cell=(self.params.dim_hog_cell, self.params.dim_hog_cell),
                                cells_per_block=(2, 2), feature_vector=False)
                num_cols = hog_descr.shape[1]
                num_rows = hog_descr.shape[0]
                num_cells_in_template = (self.params.dim_window // self.params.dim_hog_cell) - 1  # k
                for y in range(num_rows - num_cells_in_template + 1):
                    for x in range(num_cols - num_cells_in_template + 1):
                        descr = hog_descr[y: y + num_cells_in_template, x: x + num_cells_in_template].flatten()
                        score_arr = np.dot(descr, w) + bias  # sau cu decision function LinearSVC
                        # score_arr = self.best_model.decision_function(descr.reshape(1, -1))
                        score = score_arr[0]
                        if score > self.params.threshold:
                            x_min = (x * self.params.dim_hog_cell) * (1 / scale)
                            x_max = (x * self.params.dim_hog_cell + self.params.dim_window) * (1 / scale)
                            y_min = (y * self.params.dim_hog_cell) * (1 / scale)
                            y_max = (y * self.params.dim_hog_cell + self.params.dim_window) * (1 / scale)
